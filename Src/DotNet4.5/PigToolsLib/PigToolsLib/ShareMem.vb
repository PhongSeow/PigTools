...
Public Class ShareMem
        ...
        Try
            ...
            Else
                ...
                Else
                    ...
                    Me.m_pwData = ShareMem.MapViewOfFile(Me.m_hSharedMemoryFile, 2, 0, 0, DirectCast(CUInt(SMSize), UInt32))
    ...
    Public Overloads Function Read(ByRef bytData As Byte()) As String
        Try
            If mbolIsInit = False Then Err.Raise(-1, , "未初始化")
    ...
    Public Overloads Function Write(ByVal bytData As Byte()) As String
        Try
            If mbolIsInit = False Then Err.Raise(-1, , "未初始化")
            If bytData.Length <> mlngSMSize Then Err.Raise(-1, , "数据长度应为" & mlngSMSize.ToString)
