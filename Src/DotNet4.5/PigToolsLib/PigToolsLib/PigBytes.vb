...
Public Class PigBytes
    ...
    Private Const CLS_VERSION As String = "1.0.20"
    ...
    Private Function mGetValue(TypeCode As System.TypeCode, StartPos As Long, ByRef GetValue As Object) As String
        Try
            Const IS_OVER_ARRAY As String = "已经超出数组范围"
            Dim lngLen As Long = 0
            If StartPos < 0 Then StartPos = 0
            If StartPos > Me.mabMain.Length Then Err.Raise(-1, , IS_OVER_ARRAY)
            Select Case TypeCode
                Case System.TypeCode.Boolean
                    lngLen = 1
                    If Me.mIsOverBytes(StartPos, lngLen) = True Then Err.Raise(-1, , IS_OVER_ARRAY)
                ...
                Case System.TypeCode.Int64
                    ...
                    GetValue = BitConverter.ToInt32(Me.mabMain, StartPos)
                Case Else
                    Err.Raise(-1, , "暂不支持" & TypeCode.ToString)
            End Select
            mlngCurrPos += lngLen
            Return "OK"
        Catch ex As Exception
        ...
        Try
            ...
            Select Case tcAny
                ...
                Case System.TypeCode.Int64
                    abAny = BitConverter.GetBytes(SetValue)
                Case Else
                    Err.Raise(-1, , "暂不支持" & tcAny.ToString)
            End Select
    ...
    Public Function ExtByMD5(ExtCnt As Integer) As String
        ...
        Try
            ...
            For i = 0 To ExtCnt - 1
                ...
                If oGEBytes.LastErr <> "" Then Err.Raise(-1, , oGEBytes.LastErr)
    ...
    Public Function GetDeSerializeObj() As Object
        ...
        Try
            ...
            If mabMain Is Nothing Then Err.Raise(-1, , "数组未初化")
    ...
    Public Overloads Function GetBytes(StartPos As Long, GetLen As Long) As Byte()
        Try
            GetBytes = Nothing
            If StartPos < 0 Then Err.Raise(-1, , "开始位置不能小于0")
